...
def generate_data_per_query(infos_per_query, indices, file_path, file_name):
    ...
    for key in indices:
        ...
        query = interaction_info['query']
        uids = interaction_info['uids']
        url = interaction_info['docs']
        ...
        file_quid.write("{}\t{}\t{}\t{}\t{}\t{}\t{}\n".format(sid, qid, query, str(uids), url, str(vids), str(clicks)))
                ...
                for i_idx, interaction_info in enumerate(interaction_infos):
                    if qid == interaction_info['qid']:
                        ...
                        if session_uids_set == uids_set:
                            ...
                            session_print['interactions'][i_idx]['relevances'] = relevance_queries[idx][
                                'relevances'].copy()
                            break
                        else:
                            uid_match = False
...
def xml_line_removable(xml_line):
    if xml_line.find('<url>') != -1 and xml_line.find('</url>') != -1:
        return 1
